...
import { CfMotoExpenseFormDialog } from "./cf-moto-expense-form-dialog";
import type { CfMotoExpense } from "@/lib/types";

export function CfMotosDespesasView({ expenses }: { expenses: CfMotoExpense[] }) {
  const [open, setOpen] = useState(false);
  const [editingExpense, setEditingExpense] = useState<CfMotoExpense | null>(null);
  ...
  function openEdit(expense: CfMotoExpense) {
  ...
  return (
    <div className="flex flex-col gap-4 p-4 md:p-6">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold md:text-2xl">CF Motos · Despesas</h1>
      ...
      <CfMotoExpenseFormDialog open={open} onOpenChange={setOpen} expense={editingExpense} />
    </div>
  );
}
