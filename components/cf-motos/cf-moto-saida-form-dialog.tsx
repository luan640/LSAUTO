...
import {
  createCfMotoStockExit,
  updateCfMotoStockExit,
  deleteCfMotoStockExit,
} from "@/app/(app)/cf-motos/entradas/actions";
import { STOCK_EXIT_REASONS, STOCK_EXIT_REASON_LABELS } from "@/lib/types";
import type { CfMotoProduct, CfMotoStockExit } from "@/lib/types";
...
export function CfMotoSaidaFormDialog({
  open,
  onOpenChange,
  exit,
  products,
  ...
  function handleSubmit(formData: FormData) {
    ...
    startTransition(async () => {
      try {
        if (isEditing) {
          await updateCfMotoStockExit(exit.id, formData);
          toast.success("Saída atualizada");
        } else {
          await createCfMotoStockExit(formData);
  ...
  function handleDelete() {
    ...
    startTransition(async () => {
      try {
        await deleteCfMotoStockExit(exit.id);
