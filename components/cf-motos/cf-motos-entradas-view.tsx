...
import { CfMotoEntradaFormDialog } from "./cf-moto-entrada-form-dialog";
import { CfMotoSaidaFormDialog } from "./cf-moto-saida-form-dialog";
import type { CfMotoProduct, CfMotoStockEntry, CfMotoStockExit, CfMotoSupplier } from "@/lib/types";

export function CfMotosEntradasView({
  entries,
  exits,
  products,
  suppliers,
}: {
  entries: CfMotoStockEntry[];
  exits: CfMotoStockExit[];
  products: CfMotoProduct[];
  suppliers: CfMotoSupplier[];
}) {
  const [open, setOpen] = useState(false);
  const [editingEntry, setEditingEntry] = useState<CfMotoStockEntry | null>(null);
  const [saidaOpen, setSaidaOpen] = useState(false);
  const [editingExit, setEditingExit] = useState<CfMotoStockExit | null>(null);
  ...
  function openEdit(entry: CfMotoStockEntry) {
  ...
  function openEditSaida(exit: CfMotoStockExit) {
    setEditingExit(exit);
    setSaidaOpen(true);
  }
  ...
  return (
    <div className="flex flex-col gap-4 p-4 md:p-6">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h1 className="text-xl font-semibold md:text-2xl">CF Motos · Entrada de Itens</h1>
